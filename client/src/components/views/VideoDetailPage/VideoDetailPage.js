...
function VideoDetailPage(props) {
  ...
  const renderSideCards = RelatedVideos.map(relatedVideo => {
    if (!relatedVideo.isPrivate && !relatedVideo.isDeleted) {
      return (
        <SideCard
          video={relatedVideo}
        />
        ...
        <div className="w-11/12 flex flex-col justify-center divide-y">
          <div className="grid grid-cols-6 gap-4 mt-4">
            ...
            <div className="flex flex-col col-start-1 lg:col-span-4 col-span-6">
              {/* Video Player */}
              <div className="flex aspect-w-16 aspect-h-9 bg-black">
                {(!VideoDetail.isPrivate || VideoDetail.writer._id === user.userData._id) ?
                  < video
                ...
                <>
                  <div className="my-4">
                    ...
                    <div className="flex flex-row items-center justify-between space-x-3 my-4">
                      ...
                      <div className="flex">
                        {user.userData._id === VideoDetail.writer._id ?
                          <Link
                            to={`/editVideo/${videoId}`}
                            className="bg-blue-700 border-blue-800 w-max text-sm rounded px-3 py-2 border-b-4 shadow-lg text-white disabled:opacity-50"
                          >
                            Edit
                          </Link>
                          :
